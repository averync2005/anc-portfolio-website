...
function ElasticCursor() {
  ...
  const loop = useCallback(() => {
    ...
    if (isHidden || overIframeRef.current) {
      set.opacity?.(0);
    } else {
      set.opacity?.(1);
    }
  ...
  // Cross-origin iframes swallow mousemove without firing any parent event.
  // A ref (not state) avoids re-renders in the animation loop.
  const overIframeRef = useRef(false);

  // Cross-origin iframes swallow mousemove. We arm a hide timer only when the
  // last mousemove position was at an iframe's boundary — nothing else ever
  // hides the cursor. No blur/focus handlers: those fire for DevTools, address
  // bar focus, etc., which would cause spurious flicker.
  useEffect(() => {
    if (isMobile) return;
    let timer: ReturnType<typeof setTimeout>;
    const MARGIN = 4; // px from iframe edge that counts as "entering"
    const onMove = (e: MouseEvent) => {
      clearTimeout(timer);
      overIframeRef.current = false;
      const iframes = document.querySelectorAll("iframe");
      let nearIframe = false;
      iframes.forEach((iframe) => {
        const r = iframe.getBoundingClientRect();
        if (r.width === 0 || r.height === 0) return; // skip collapsed/hidden iframes
        if (
          e.clientX >= r.left - MARGIN && e.clientX <= r.right  + MARGIN &&
          e.clientY >= r.top  - MARGIN && e.clientY <= r.bottom + MARGIN
        ) nearIframe = true;
      });
      if (nearIframe) timer = setTimeout(() => { overIframeRef.current = true; }, 200);
    };
